...
def add_to_cart(db):
    ...
    if quantity and quantity > 0:
        existing_item = cart.query.filter(
            and_(cart.product_name == name, cart.user_id == user_id)).first()
        if existing_item:
            existing_item.product_quantity = quantity
            db.session.commit()
            return jsonify({"success": True})
        else:
            new_item = cart(product_name=name, product_cost=netcost,
                            product_quantity=quantity, status_id=3, user_id=user_id)
...
def update_cart_item_quantity(db):
    ...
    cart_item = cart.query.filter_by(
        product_name=productName, user_id=user_id).first()
